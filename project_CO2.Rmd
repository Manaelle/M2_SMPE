...
summary(linear_reg)
resid(linear_reg) #List of residuals
plot(density(resid(linear_reg))) #A density plot
qqnorm(resid(linear_reg)) # A quantile normal plot - good for checking normality
qqline(resid(linear_reg))
```
```{r}
mean(clean_data$CO2, na.rm=TRUE)
```
...
The figure from the table above shows a continuous CO2 concentration increasing. It seems to be linear. So let's start with a linear regression.
...
__F-Statistic__ :  checks if at least one variable’s weight is significantly different than zero. More used to compare models.
